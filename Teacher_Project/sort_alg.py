...
my_list = [rd.randint(-500, 1000) for _ in range(1_000_000)]
...
def bubble_sort_standart(arr: list):  # стандартне сортування бульбашкою
    ...
    for i in range(l):  # цикл для кожного елементу, що буде рухатись
        for j in range(0, l - i - 1): # цикл тільки для елементів, що знаходяться ЗПРАВА від нашого (- 1, щоб не включити останній)
            if arr[j] > arr[j + 1]:
...
def merge_sort(arr: list):
    ...
    if j < len(right):  # аналогічно з правою частиною
        res.extend(right[j:])
...
start = time.time()
res1 = merge_sort(my_list)
end = time.time()
print(f'Результат merge sort: {end - start}')

start = time.time()
res2 = quick_sort(my_list)
end = time.time()
